 ...
 *
 *
 * 一个常见的Spring注释，用于声明在某些情况下带注释的元素可以是{@code null}。 利用JSR 305元注释来指示Java中的可空性，以及JSR 305支持的常用工具，并由Kotlin用于推断Spring API的可空性。
 *
 * <p>应在参数，返回值和字段级别使用。 方法覆盖应重复父{@code @Nullable}注释，除非它们的行为不同。
 *
 * <p>可以与{@code @NonNullApi}或{@code @NonNullFields}结合使用，将默认的非可空语义覆盖为可空。
 *
